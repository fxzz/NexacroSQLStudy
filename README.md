...
## 목적
SQL 및 Nexacro 의 숙련도 향상을 위해 ChatGPT를 활용하여 테이블 학습을 수행하고, 요구사항을 요청해 전문성을 키우는것.

<br><br>
### 2023.12.11 ~ 2023.12.11
### 19. 요구사항: "특정 제품의 주문량과 매출 조회"

```
1. 기능:
사용자는 특정 제품의 주문량과 매출을 조회할 수 있는 기능을 사용할 수 있어야 합니다.

2. 화면 요소:
"제품 조회" 입력 필드: 조회할 제품의 이름이나 코드를 입력하는 필드입니다.
"조회" 버튼: 입력된 제품에 대한 주문량과 매출을 조회하는 버튼입니다.
결과를 표시할 테이블: 조회된 주문량과 매출 정보를 표시하는 테이블입니다.

3. 동작:
사용자가 "제품 조회" 입력 필드에 제품의 이름이나 코드를 입력하고, "조회" 버튼을 클릭하면, 시스템은 해당 제품의 주문량과 매출을 조회하여 결과를 표시합니다.
테이블에는 제품명, 주문량, 매출 금액 등이 포함되어야 합니다.

```

![요구사항](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/ed9dff6e-f105-4052-b3c1-5cc5b1618935)


![쿼리](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/6c01e1bb-14f9-4762-9238-af3410869e9c)


<br><br>
### 2023.12.11 ~ 2023.12.11
### 18. 요구사항: "매출 순위 10위 까지 조회"

```
1. 기능:
사용자는 전체 고객 중 매출이 높은 상위 10개 고객의 매출 정보를 조회할 수 있는 기능을 사용할 수 있어야 합니다.

2. 화면 요소:
상위 10위 매출 조회" 버튼: 상위 10개 고객의 매출 정보를 조회하는데 사용합니다.
결과를 표시할 테이블: 상위 10개 고객의 매출 정보를 표시하는 테이블입니다.


3. 동작:
사용자가 "상위 10위 매출 조회" 버튼을 클릭하면, 시스템은 전체 고객 중 매출이 높은 상위 10개 고객의 매출 정보를 계산하여 결과를 표시합니다.
테이블에는 각 고객의 매출 순위, 고객 아이디, 매출 금액 등이 포함되어야 합니다.
결과는 매출 금액이 높은 순서로 정렬돼야 하며, 상위 10개만 표시되어야 합니다.

```

![요구사항](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/481e3dce-4269-475b-9039-c0851d1fe6dc)

![쿼리](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/f963aae7-9632-4588-95bf-5aaef44ee1ee)


<br><br>
### 2023.12.10 ~ 2023.12.10
### 17. 요구사항: "특정 연도 동안 주문이 없는 고객 조회 쿼리"

```
1. 기능:
사용자는 특정 연도 동안 주문을 하지 않은 고객들을 조회할 수 있는 기능을 사용할 수 있어야 합니다.

2. 화면 요소:
"주문 없는 고객 조회" 화면
조회 연도를 입력하는 텍스트 상자
"조회" 버튼
결과를 표시할 그리드 또는 리스트: 주문이 없는 고객 정보를 표시

3. 동작:
사용자는 조회 연도를 지정하여 "조회" 버튼을 클릭합니다.
시스템은 해당 기간 동안 주문을 하지 않은 고객들을 조회하여 결과를 표시합니다.
결과에는 고객 아이디, 마지막 주문일, 거주 도시, 전화번호 등이 포함되어야 합니다.

```

![요구사항](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/c5128bac-7780-44e3-a6dc-5642338fefcc)

![쿼리](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/6d1adddf-ba99-4d85-b63e-2dbb5fbbf6b4)
...
<br><br>
### 2023.12.10 ~ 2023.12.10
### 16. 요구사항: "고객이 마지막 일자로 주문한 상품 정보 조회"

```
1. 기능:
사용자는 특정 고객이 마지막 일자로 주문한 상품 정보를 조회할 수 있는 기능을 사용할 수 있어야 합니다.

2. 화면 요소:
"고객 주문 조회" 화면
"고객 입력" 조회할 고객을 입력하는데 사용
"조회" 버튼: 선택한 고객의 마지막 주문 상품 정보를 조회하는데 사용
결과를 표시할 그리드 또는 리스트: 고객의 마지막 일자로 주문 상품 정보를 표시

3. 동작:
사용자는 고객을 입력하고 조회 합니다.
"조회" 버튼을 클릭하면, 시스템은 선택한 고객이 마지막 일자로 주문한 상품 정보를 조회하여 결과를 표시합니다.
결과에는 주문 번호, 주문 일자, 상품명, 수량, 주문 금액 등이 포함되어야 합니다.
조회된 결과는 과거 금액이 높은 순서대로 나열돼야 합니다.
```

![요구사항](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/e7ccd9d6-a6df-4574-86b8-a98953164404)


![쿼리](https://github.com/fxzz/NexacroSQLStudy/assets/3148006/0a5304d6-f2f3-481f-bdb1-e788f8fc0f35)
